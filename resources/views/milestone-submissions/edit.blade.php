        ...
        <div class="col-md-8">
            <div class="card shadow-sm">
                <div class="card-header bg-warning text-dark">
                    <h5 class="mb-0">Chỉnh sửa Bài Nộp</h5>
                </div>
                <div class="card-body">
                    ...
                    <form action="{{ route('milestone-submissions.update', $milestoneSubmission->id) }}" method="POST">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <select name="milestone_id" class="form-control @error('milestone_id') is-invalid @enderror" required>
                                ...
                                @foreach($milestones as $milestone)
                                    <option value="{{ $milestone->id }}" {{ old('milestone_id', $milestoneSubmission->milestone_id) == $milestone->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <select name="student_id" class="form-control @error('student_id') is-invalid @enderror" required>
                                ...
                                @foreach($students as $student)
                                    <option value="{{ $student->id }}" {{ old('student_id', $milestoneSubmission->student_id) == $student->id ? 'selected' : '' }}>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="text" name="file_path" class="form-control @error('file_path') is-invalid @enderror" 
                                   value="{{ old('file_path', $milestoneSubmission->file_path) }}" required>
                            @error('file_path') <div class="invalid-feedback">{{ $message }}</div> @enderror
                        </div>

                        <div class="mb-3">
                            <label class="form-label fw-bold">Ghi chú</label>
                            <textarea name="note" class="form-control @error('note') is-invalid @enderror" rows="3">{{ old('note', $milestoneSubmission->note) }}</textarea>
                        ...
                        <div class="mb-3">
                            ...
                            <input type="datetime-local" name="submitted_at" class="form-control @error('submitted_at') is-invalid @enderror" 
                                   value="{{ old('submitted_at', $milestoneSubmission->submitted_at->format('Y-m-d\TH:i')) }}" required>
